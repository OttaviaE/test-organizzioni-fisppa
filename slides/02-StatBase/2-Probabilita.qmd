...
`*` sta per nome della distribuzione in R, per esempio `rnorm` per la
Normale.
...
`dnorm()` restituisce la **densità** in un punto, cioè l'altezza della
curva in quel punto. Per le distribuzioni continue, la densità **non è
una probabilità**: la probabilità di un singolo valore esatto è sempre
zero.
...
Dato un valore $q$, restituisce l'area a sinistra sotto la curva, cioè
$P(X \leq q)$.
...
# Probabilità di ottenere un punteggio >= 60 (corrisponde a z = 2)
pnorm(60, mean = 40, sd = 10, lower.tail = FALSE)

# È identico a lavorare direttamente sulla Normale standard:
pnorm(2, lower.tail = FALSE)

# Probabilità di punteggio compreso tra 30 e 50 (media +/- 1 sd)
...
Dato un livello di probabilità cumulata $p$, restituisce il quantile
corrispondente, cioè il valore $x$ tale che $P(X \leq x) = p$. È
l'inversa di `pnorm()`.
...
# Sulla Normale standard: valore z al 97.5° percentile
qnorm(0.975)
```
...
9.  Qual è il **rango percentile** di un lavoratore con punteggio 35? E
    di uno con punteggio 70?

10. Un lavoratore ottiene un punteggio di 80. Dove si colloca il suo
    punteggio? Quanti lavoratori ci aspettiamo abbiano un punteggio
    uguale o superiore?
