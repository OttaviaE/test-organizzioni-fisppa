...
abline(v = mean(exp_totali), col = "orange", lwd = 6, alpha = 0.5)       # media
...
```{r, echo=FALSE,fig.width=6, fig.height=2.6}
set.seed(101)
x1  <- rgamma(200, shape = 2, rate = 0.5)

ggplot(data.frame(x = x1), aes(x = x)) +
  geom_histogram(fill = "steelblue", color = "white",
               alpha = 0.7, linewidth = 1.5) +
  labs(x = "var", y = "frequenza", color = "") +
  theme(legend.position = "top")
...
ggplot(data.frame(x = x1), aes(x = x)) +
  ...
  theme(legend.position = "top")+
  scale_x_continuous(breaks = as.numeric(round(quantile(x1),2))[2:4])
...
```{r, echo=FALSE,fig.width=3, fig.height=2.6}
x2  <- 20 - rgamma(100, shape = 2, rate = 2)

ggplot(data.frame(y = x2), aes(y = y)) +
  geom_boxplot(fill = "steelblue", color = "black",
               alpha = 0.7, linewidth = 1) +
  labs(y = "var", x = " ", color = "") +
  scale_x_continuous(breaks = seq(-1,1,1), labels = c("",""," "))+
  theme(legend.position = "top")
...
df_k <- data.frame(
  x    = c(rnorm(n_k, 0, 1),
           rt(n_k, df = 2),
           runif(n_k, -3, 3)),
  Tipo = rep(c("A", "B", "C"), each = n_k)
)

ggplot(df_k, aes(x = x, color = Tipo, fill = Tipo)) +
  geom_density(alpha = 0.18, linewidth = 1.3, adjust = 1.5) +
  scale_color_manual(
    values = c("A" = "orchid3", "B" = "steelblue", "C" = "orange")) +
  scale_fill_manual(
    values = c("A" = "orchid3", "B" = "steelblue", "C" = "orange")) +
  xlim(-6, 6) +
  labs(x = "Valori", y = "Densità",
       color = "Distribuzione", fill = "Distribuzione") +
  theme(legend.position = "top")
