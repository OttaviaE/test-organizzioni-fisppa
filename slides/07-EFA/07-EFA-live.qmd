...
Considerando $i = 1, \ldots I$ le variabili manifeste (item) e $p = 1, \ldots, P$ e assumendo $\theta$ come fattore latente unidimensionale:
...
$\theta_p$: è il livello di tratto latente *non osservator* per il $p$-esimo soggetto 
...
$\theta_1, \ldots, \theta_F$, presupponendo che comunque $F < I$: 
...
Verifica se la matrice di correlazione $\mathbf{R}$ è significativamente diversa da una matrice identità $\mathbf{I}$ (ovvero una matrice dove è prevista l'indipendenza tra tutte le variabili).
...
|Metodo|	Logica|
|:------ |---------:|
|Kaiser (eigenvalue > 1)|	soglia fissa|
|Scree | plot	giudizio visivo |
|Parallel analysis|	confronto con dati casuali|
...
Misura quanta della varianzaz totale della matrice di correlazione è catturata da ogni fattore $f$
...
Basato sulla verosimiglianza $\mathcal{L}$ ottenuta dal confronto tra la matrice di covarianza osservata e la matrice di covarianza attesa ($cov$) secondo il modello fattoriale ($\hat{cov}$)

$H_0$: $cov = \hat{cov}$

$df$: Numero di elementi nella matrice di covarianza $-$ numnero di parametri stimati (saturazioni, correlazioni tra fattori ecc.)
...
Il CFI è un indice che è stato pensato per modelli confermativi 
