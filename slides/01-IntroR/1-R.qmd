...
```{r,echo=TRUE}
head(subset(my_df, subset = lettere == "a" & numeri > 2, 
       select = c(lettere, numeri)), n = 3)
```
...
oppure la funzione **`_join()`**:

```{r, echo=TRUE, message=FALSE}
library(dplyr) # carico il pacchetto dplyr

df_all_2 = left_join(x = df_rt, y = df_age, by = c("subj")) 
str(df_all_2)
```
...
E' consigliato avere una versione del dataset in formato .csv e un'altra in formato .rda (formato specifico di R)
...
```{r,echo=TRUE,message=FALSE}
rt_csv$subj = as.factor(rt_csv$subj)
rt_csv$cond = as.factor(rt_csv$cond)
str(rt_csv)
```
